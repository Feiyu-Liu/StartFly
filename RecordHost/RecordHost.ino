...
// ------------------ 外部中断 (INT0, D2) ------------------
void triggerISR() {
  lastTriggerStamp = TCNT1; // 读取Timer1当前计数值
  hasNewTrigger = true;     // 设置新触发标志
}
...
void setup() {
  ...
  // ---- 配置外部中断 INT0 (D2) ----
  // 使用标准的 attachInterrupt, 绑定 D2 的上升沿到 triggerISR
  attachInterrupt(digitalPinToInterrupt(TRIGGER_TTL_PIN), triggerISR, RISING);
